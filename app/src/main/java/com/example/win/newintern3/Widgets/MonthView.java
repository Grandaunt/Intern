...
import com.example.win.newintern3.Entity.DPInfo;
...
public class MonthView extends View {
    ...
    private int sizeDecor, sizeDecor2x, sizeDecor3x, sizeDecor13x,sizeDecor15x,sizeDecor23x ,sizeDecor25x;
    ...
    protected void onSizeChanged(int w, int h, int oldW, int oldH) {
        ...
        //尺寸设计：
        sizeDecor13x = (int)(sizeDecor * 1.4);
        //尺寸设计：102
        sizeDecor2x = sizeDecor * 2;
        //尺寸设计：
        sizeDecor23x = (int)(sizeDecor * 2.4);
        //尺寸设计3：153
        sizeDecor3x = sizeDecor * 3;
        //尺寸设计：
        sizeDecor15x = (int)(sizeDecor * 1.7);
        //尺寸设计3：
        sizeDecor25x = (int)(sizeDecor * 2.7);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        draw(canvas, (width-10) * indexMonth, indexYear * height, centerYear, centerMonth);
...
//
    ...
    private void drawBGToday(Canvas canvas, Rect rect) {
        ...
        canvas.drawCircle(rect.centerX(), rect.centerY()+20, circleRadius / 3F, mPaint);
    }
   //节日的背景色
    ...
    private void drawGregorian(Canvas canvas, Rect rect, String str, boolean isWeekend,boolean isToday) {
        ...
        if (!isFestivalDisplay)
            y = rect.centerY() + Math.abs(mPaint.ascent()) - (mPaint.descent() - mPaint.ascent()) / 12F;
    ...
    private void drawDecor(Canvas canvas, Rect rect, DPInfo info) {
        if (!TextUtils.isEmpty(info.strG)) {
            ...
            if (null != mDPDecor && info.isDecorTR) {
                canvas.save();
                canvas.clipRect(rect.left + sizeDecor15x, rect.top+sizeDecor, rect.left + sizeDecor25x,
                        rect.top + sizeDecor15x);
            ...
            if (null != mDPDecor && info.isDecorB) {
                canvas.save();
                canvas.clipRect(rect.left + sizeDecor13x, rect.top+sizeDecor23x, rect.left + sizeDecor15x,
                        rect.top + sizeDecor25x);
                mDPDecor.drawDecorT(canvas, canvas.getClipBounds(), mPaint, data);
                canvas.restore();
            }
        }
