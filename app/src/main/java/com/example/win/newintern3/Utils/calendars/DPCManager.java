package com.example.win.newintern3.Utils.calendars;
...
//        DECOR_CACHE_R.clear();
    ...
    private DPInfo[][] buildDPInfo(int year, int month) {
        ...
        Set<String> decorB = DECOR_CACHE_B.get(year + ":" + month);
        for (int i = 0; i < info.length; i++) {
            for (int j = 0; j < info[i].length; j++) {
                ...
                if (null != decorB && decorB.contains(tmp.strG)) tmp.isDecorB = true;
                info[i][j] = tmp;
            }
        }
        return info;
