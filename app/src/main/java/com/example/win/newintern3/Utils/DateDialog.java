...
//        }
    ...
    public int setList(final String title, final TextView resultText,final TextView telText,final TextView teacherId, final String[] datas,final ServiceListEntity<TeacherInfo> list) {
        post = 0;
        dialog = new AlertDialog.Builder(context);
        dialog.setTitle("选择教师");
        dialog .setItems(datas, new DialogInterface.OnClickListener()
        {
            ...
            public void onClick(DialogInterface dialog, int which)
            {
                ...
                telText.setText(list.getList().get(which).getTeacherTel());
                resultText.setText(datas[which]);
                teacherId.setText(list.getList().get(which).getTeacherId().toString());
            }
        });
        dialog.show();
