...
import com.example.win.newintern3.Utils.MeasureUtil;
import com.example.win.newintern3.Utils.decors.DPDecor;
import com.example.win.newintern3.Utils.languages.DPLManager;
import com.example.win.newintern3.Utils.themes.DPTManager;
...
public class DatePicker extends LinearLayout {
    ...
    private ImageView imRight,imLeft;//年月日向左向右
...
//        rlTitle.setPadding(rlTitlePadding, rlTitlePadding, rlTitlePadding, rlTitlePadding);
        ...
        int rlTitlePadding = MeasureUtil.dp2px(context, 20);
        rlTitle.setPadding(rlTitlePadding, rlTitlePadding, rlTitlePadding, rlTitlePadding);
        // 周视图根布局
        ...
        int llWeekPadding = MeasureUtil.dp2px(context, 2);
        llWeek.setPadding(25, llWeekPadding, 25, llWeekPadding);
        ...
        lpleft.addRule(RelativeLayout.CENTER_IN_PARENT);
        RelativeLayout.LayoutParams lpRight =
                new RelativeLayout.LayoutParams(WRAP_CONTENT, WRAP_CONTENT);
        lpRight.addRule(RelativeLayout.CENTER_IN_PARENT);
        RelativeLayout.LayoutParams lpYear =
                new RelativeLayout.LayoutParams(WRAP_CONTENT, WRAP_CONTENT);
        lpYear.addRule(RelativeLayout.CENTER_IN_PARENT);
        ...
        // --------------------------------------------------------------------------------标题栏
        //左图标
//        imRight = new ImageView(context);
//        imRight.setImageResource();
        ...
        for (int i = 0; i < mLManager.titleWeek().length; i++) {
            ...
            tvWeek.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 16);
            if(i==0||i>=6){
                tvWeek.setTextColor(mTManager.colorToday());
            }
            else {
